...
### UI 质感提升执行清单（品牌辨识度，2026-08-09 融合落盘）

> 来源：`EmberInn-UI质感提升方案`。问题诊断：功能流畅、M3 规范，但"任何一个 M3 App"的既视感。**根本原因**：处处用 M3 组件默认值（默认圆角/阴影/字体/算法直出配色），规范只保证"正确"，不保证"有态度"。本清单是在规范之上加"只属于 EmberInn 的偏好层"。
> 验收自查：①截图盲测——和任意有质感的 App 并排对比阴影软硬/图标粗细/间距节奏/饱和度；②去标识测试——截图挡住 Logo 认不出是自己做的 = 品牌指纹未成型。
...
10. **算法取色后加"品牌滤镜"**：MaterialKolor 输出后统一降饱和一档 + 背景叠专属冷灰/暖灰 tint，让任何 seed 都带 EmberInn 气质
11. **空状态设计**：替换"暂无数据"默认文案/灰图标，用余烬/炭火意象（将熄未熄的炭火）+ 有语气文案，覆盖首页无卡、聊天无记录
12. **设置页重新设计**：避免"图标+文字+箭头"机械通栏，按语义分组独立卡片区块、组间留白、分组标题品牌化

**第三批（品牌母题与排版）**
13. **统一品牌视觉母题"炭火/余烬"**：不止图标，复用到加载指示器（火星微光替代转圈）、下拉刷新、空状态插画、发送成功反馈——同一符号多形式复现
14. **排版层级大胆**：标题/正文字号级差、字重跳跃更鲜明（Bold 直跳 Light），层级对比 = 经过设计
15. **图标一致性**：全局排查混用 `Icons.Default.xxx`（Material 内置）与 Phosphor 的情况，统一 Phosphor
