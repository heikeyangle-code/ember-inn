...
**剩余已知半成品（继续治理中）**：工具调用 App 注册表、斜杠异步命令（/gen /genraw /while，需异步执行器）、表情精灵 App 层、preset 正则存储/UI、ComfyUI 图像来源（需 workflow 管理）。
