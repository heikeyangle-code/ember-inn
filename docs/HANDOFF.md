...
**尚未做差分的**：斜杠解析器（SlashCommandParser 依赖数十个模块与 DOM，无法逐字提取；手写单测 + 源码对照）、CharX/BYAF 导入（官方依赖 JSZip/文件系统，手写单测）、preparePromptsForChatCompletion（populateChatHistory 5 例 + populateDialogueExamples 4 例已差分；preparePrompts 依赖全局状态，单测 + 源码对照）。
...
- 引擎测试 195 全绿（含重排/文件/分块/工具函数/作用域宏/YAML 导入）
