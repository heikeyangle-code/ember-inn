...
- 引擎测试与 App Kotlin 编译本机均可跑（Java 17.0.2 + Gradle 9.7.0 + Android SDK 34，当前 **567 例全绿**：381 testcase + imagegen-services 90 fixture / tts-services 37 fixture / tts-local 38 fixture 各单 testcase 内 for-loop）；完整 APK 组装/签名走 CI。注意：Termux 本机无 gradle，本地验证 = `cd scripts/kernel-golden && npm test`（金测）+ 差分脚本 node 重生成 fixture；Kotlin 编译/单测以 CI 为准。
...
**差分分组清单（97 行，表内合计 3040 例；历史 85/1969 为旧口径）见 [docs/DIFF_MATRIX.md](DIFF_MATRIX.md)，含打桩/未差分登记；新增 message-formatting-official.mjs → MessageFormattingDiffTest 805 例、cfg-prompt 25 例、logprobs 20 例、imagegen 10 例、imagegen-services-official.mjs → ImageGenServicesDiffTest 63 例（9 云端后端 + 5 LLM 后端 + replaceComfyWorkflow 纯函数 5 例（官方带引号占位符语义）+ comfy-seed-resolve 4 例 + getClosestSize 工具函数 4 例）、tts-services-official.mjs → TtsServicesDiffTest 37 例（11 云端后端 + Electron Hub；volcengine speed 0 透传 2 例）、tts-local-official.mjs → TtsLocalDiffTest 38 例（13 本地后端）、chat-template-official.mjs → ChatTemplateDiffTest 25 例、model-sort-official.mjs → ModelSortDiffTest 48 例。**
...
### 3.12 图像生成（stable-diffusion） ✅（17 后端差分全量 90 例：9 云端 + 5 LLM + replaceComfyWorkflow + novel/drawthings/分辨率模式）
...
- **✅ 5 LLM 后端 + replaceComfyWorkflow 纯函数（差分 3→39→63→90）**：falai-server（服务端加工后 requestBody，4 例）/ google-client（客户端 body，2 例）/ zai（客户端 body，2 例）/ openrouter（客户端 body，3 例）/ workersai-client（客户端 body，4 例）/ replaceComfyWorkflow（官方带引号占位符语义 5 例）+ resolveComfySeed（seed>=0 原样否则 Math.round(random01*MAX_SAFE) 打桩 4 例）；引擎层新增 `falaiServerBody`/`googleClientBody`/`zaiClientBody`/`openRouterBody`/`workersAiClientBody`/`replaceComfyWorkflow`（含 `bflBody` 共用的 `numStr` 数字语义 helper）。
- **✅ 第四批：直连源架构 + 分辨率/生成模式（差分 63→90）**：①**逐源 URL/auth**——官方 getSdRequestBody 每源独立 URL+Basic 凭据（auto/vlad/sdcpp/drawthings/comfy/comfy_runpod 六组设置键），App [ImageGenClient.kt](file:///workspace/app/src/main/java/com/emberinn/app/data/ImageGenClient.kt) per-source when 分发 + `basicAuthHeader`（`"Basic "+base64`，对齐服务端 util.js 空凭据也恒发）；②**pingSource 验证语义 1:1**——auto/vlad GET 整路径替换 /sdapi/v1/options、sdcpp OPTIONS urlJoin /v1/images/generations、drawthings HEAD '/'、comfy GET /system_stats、comfy_runpod GET /health Bearer（缺 key→400 "RunPod key not found."；workers.ready<=0 警告仍 200），设置页每源“测试连接”；③**DrawThings 直连**——引擎 `drawthingsPayload` 官方键序全字段（prompt/negative/sampler_name/steps/cfg_scale/width/height/restore_faces/enable_hr/denoising_strength/clip_skip 可空省略/upscaler_scale/seed>=0）差分 3 例；④**分辨率预设 + snap**——官方 30 项有序分辨率表 RESOLUTION_OPTIONS + getClosestKnownResolution（aspectDiff+resolutionDiff 最小化，严格 < 保并列首个，差分 5 例）+ setTypeSpecificDimensions（FACE/FACE_MULTIMODAL 竖版 1.5 倍、BACKGROUND 横版 1.8 倍、媒体附件尺寸直用、64 取整后 snap，差分 6 例）；App 设置页分辨率 MenuPicker（onSelect 先写 height 后写 width 对齐官方 onResolutionChange）+ sd_snap 开关；生成时按 generationMode（FACE=5/BACKGROUND=7/FACE_MULTIMODAL=10）调尺寸，临时写 prefs 生成后 finally 恢复（对齐官方 restoreDimensions）；⑤**NovelAI 参数开关 UI**——variety+/decrisper/sm/sm_dyn 设置页补齐；⑥**Horde sanitize/nsfw/karras 开关**补齐；⑦**Stability style preset / Pollinations enhance / BFL upsampling / HF model id / minimal_prompt_processing 开关**补齐；⑧**noCharPrefix**（index.js L3318：USER/FREE/BACKGROUND/USER_MULTIMODAL/FREE_EXTENDED 模式不拼角色前缀；swipe 豁免 App 无对应入口）；⑨**角色图像提示词编辑器**——CharacterDetailScreen“图像提示词（该卡）”卡片，正/负前缀按角色 id 存 ServicesPrefs（官方 character_prompts/character_negative_prompts），生成时经 extraPrompt/negativePrompt 合并。⑩**Shareable 共享**——详情页“共享到角色卡”开关：勾选把 {positive,negative} 写入角色卡 data.extensions.sd_character_prompt（CharacterCardEdit.applySdCharacterPrompt，官方 writePromptFields/writeExtensionField 语义），进页时卡上有共享数据且本地为空则导入（onChatChanged L883-L898）。
- **✅ Comfy 全量对齐（generateComfyImageCommon L4231-L4261 + 服务端 /generate L562-L630）**：占位符搜索串带外层双引号 '"%prompt%"'、替换值 JSON.stringify 语义；%denoise%（undefined→1.0）/%clip_skip%（isNaN→-1 否则取负）/%vae%/%sampler%/%scheduler% 补齐；runPod 标志仅 steps/scale/width/height 4 项；删自创 model 兜底 v1-5-pruned-emaonly。App 直连 ComfyUI：提交 body = 官方模板字符串原样 '{"prompt": <workflow>}'（服务端 request.body.prompt 直传）；outputs images 平铺优先→gifs；/view query 不编码。设置页已补 comfy_placeholders 自定义替换编辑器（标准键清单 ✅/❌ 标记 + 自定义 find/replace 行增删，对齐官方 comfy placeholder 替换链）；%user_avatar%/%char_avatar% 头像注入已接（激活人格头像 + 当前角色头像转 base64 注入带引号占位符，读取失败回退官方 PNG_PIXEL；comfy 与 comfy_runpod 两路同效，对齐 generateComfyImageCommon L4251-L4271）。
- **✅ NovelAI/Horde/OpenAI 直连请求体逐项对齐**：NovelAI seed=sd_seed>=0 原样否则 Math.floor(rand*9999999999)（原恒随机且上界错）、负面链真实传递、scheduler 白名单归一 karras、use_order 移入 v4_prompt 内、model 对 '' 不兜底；Horde 参数全读设置 + karras 官方怪点恒 false（客户端从不发该字段）+ seed 仅 >=0 发送 + steps 无截断 + models 无 Deliberate 兜底；OpenAI model 读 sd_model + 模型族截断（1000/4000/32000）+ size 宽高比逻辑（1792/1536/512）+ quality/style/moderation 条件发送。variety+/decrisper/sm/sm_dyn 与 Horde sanitize/nsfw/karras 开关均已上设置页（第四批）。登记偏差：NovelAI upscale 二次调用未接；OpenAI sora-2 视频分支未接。
- **✅ /imagine 全量参数覆盖（applyCommandArguments L5384-L5408）+ combinePrefixes**：aliases sd/img/image；16 个设置类命名参数（seed/width/height/steps/cfg/skip/model/sampler/scheduler/vae/upscaler/scale/hires/denoise/2ndpass/faces）临时写 prefs 生成后 finally 恢复；失败返回 ''（对齐官方 returns）；提示词组装改官方 combinePrefixes（', ' 连接去首尾逗号、{prompt} 宏首处原位替换、负面链顺序 = 命令附加→设置→角色负面）。登记偏差：quiet/gallery 聊天展示参数属接线层未区分；width+height 同时给时回写 mediaAttachment 尺寸元数据未做（MediaAttachment 模型缺 width/height 字段）；生成中断 AbortSignal 链路未接（官方 socket close → ComfyUI /interrupt、RunPod /cancel、Horde delete 请求）。
- **✅ App 层 LLM 全部改接线（准则 2）**：[ImageGenBackendsLlm.kt](file:///workspace/app/src/main/java/com/emberinn/app/data/ImageGenBackendsLlm.kt) 8 LLM 后端——Google/ZAI/OpenRouter/WorkersAI/FalAI/Extras/ComfyRunPod/DrawThings——均只做 HTTP 接线：①从 ServicesPrefs 取参数构造纯值；②调引擎层对应方法得 JsonObject body；③拼 URL + Header + 发请求 + 响应解析（落盘 saveBase64/saveFromUrl/saveFromRunPodOutput）。Google Vertex 客户端翻译为 `{instances,parameters}` Vertex predict 契约、WorkersAI Cloudflare form 契约、ComfyRunPod `/run`→`/status/{id}` 轮询均属接线（不在差分范围，差分覆盖的纯函数 1:1 已保）；DrawThings 登记不实现。
...
1. **第 16 阶段 Polish ✅ 全部完成**：Typography 审计=12 档类型比例表
   ...
   fade+上滑走 pageMs（减动画降级纯 fade），ShellKit 开关滑块、Editorial AccordionGroup、Onboarding 分段入场全部归档；保留值均有据：
...
2. **第 17 阶段回归 ✅ 机检全过，剩真机人工项**：内核金测试 jsdom 四套件
   ...
   导入导出回归。
3. V3 已落要点（速查）：三空间壳（EmberBottomNav 底部导航四目的地，宽屏 HubRail 轨）；书架排序 11 档/标签筛选/网格⇄列表（char_list_grid）；群聊生成队列条（show_group_chat_queue）；角色主页 CharacterHomeScreen + 编辑器分离；聊天 Context 胶囊；设置 IA 官方 8 分区 + 全域搜索目录；BehaviorPrefs 35 字段全链路核销（见 V3 文档 §2.2.2 表）；自适应三档（<600 单栏 / 600-840 限宽 600dp / ≥840 导航轨 + List-Detail，windowWidthClass() 自实现）；壳层个性化三键（shell_density/motion_level/home_style）；主题切换 lerp 400ms（EmberColors.lerpTo）。
...
- 引擎测试 **全绿**（engine 全量 testcase + imagegen-services 90 + tts-services 37 + tts-local 38 + quickreply 16 + caption 17 + translate 19 + gallery-assets 5 + imagegen-prompt 59 → fixture 内 for-loop 累计）；App `:app:compileDebugKotlin` 全过；App 单测 `GalleryAssetsDiffTest` 5 例全绿；差分分组表 104 组，明细见 [docs/DIFF_MATRIX.md](DIFF_MATRIX.md)。
- 完成项对齐（“写了未差分”全部补齐）：
