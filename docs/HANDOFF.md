...
- 引擎测试 **508 例全绿**（原 490 + imagegen-services 39→57 例扩 18；381 testcase + imagegen-services 57 fixture / tts-services 35 fixture / tts-local 38 fixture 各单 testcase 内 for-loop）；差分分组表 97 行（表内例数合计 3040；历史"85 组/1969 例"为旧口径，不再使用），明细见 [docs/DIFF_MATRIX.md](DIFF_MATRIX.md)。
- 剩余未做：图像生成 ADetailer / 样式库 / prompt templates / refine·interactive·multimodal 模式 / Comfy workflow 管理（见 3.12 登记）；TTS 3 本地后端登记不差分（kokoro/kokoro-worker/openai-compatible，不同源见 4.4）；Caption extras/local/horde 来源路由（接线为主，无可差分纯 body）；Translate reasoning 自动翻译（接线为主）；未声明 closureArgs 的闭包仍即时求值（SlashEngine，见 3.4）；斜杠命令按用户决策裁剪（仅高价值命令，见 3.4）；发送链路未接项（见 6.2 登记）；自定义预设"设为默认"（官方无此概念）。
- **写了未差分（违规）修复进展**：①图生 17 新后端 **全量差分**（9 云端 + 5 LLM 后端 + replaceComfyWorkflow 纯函数，imagegen-services-official.mjs 3→39→57 例 + 引擎层 `ImageGenRequestEngine` 提取 + `ImageGenServicesDiffTest`，App 全 17 后端改接线）；②TTS 27 外部后端 **24/27 差分**（11 云端 + 13 本地，tts-services-official.mjs 35 例 + tts-local-official.mjs 38 例，App 改接线；3 本地登记不差分因不同源 WebWorker/ST 代理）；③attachments/gallery/assets/quick-reply App 服务（6.1 登记）**接线为主，强 Android 依赖留 App 不提取引擎**（CRUD/排序/缩略图/持久化都属 App，无可差分纯函数等价官方 body 构造）。当前 `:engine:test` 全过 + `:app:compileDebugKotlin` 全过。
...
| attachments / gallery / assets / quick-reply App 服务 | app/data/AttachmentsService.kt（三源 CRUD）/ GalleryService.kt（四排序+视频缩略图）/ AssetsService.kt（5 类型资产）/ QuickReplyStore.kt（多预设迁移）已写完对照官方 `extensions/{attachments,gallery,assets,quick-reply}/index.js`；**强 Android 依赖，留 App 接线**（CRUD/排序/缩略图/持久化都属 Android 存储层，无可差分纯函数等价官方 body 构造；Attachments CRUD 若后续有纯字段归一/互转需求再引擎化）；AppSlashExecutor `/db` `/listGallery` `/installAsset` `/deleteAsset` `/qr` 已桥接到这些服务 | 🟡 接线为主，引擎无纯函数可差分 |
