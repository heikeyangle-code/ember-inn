...
- **✅ 5 LLM 后端 + replaceComfyWorkflow 纯函数（差分 3→39→63）**：falai-server（服务端加工后 requestBody，4 例）/ google-client（客户端 body，2 例）/ zai（客户端 body，2 例）/ openrouter（客户端 body，3 例）/ workersai-client（客户端 body，4 例）/ replaceComfyWorkflow（官方带引号占位符语义 5 例）+ resolveComfySeed（seed>=0 原样否则 Math.round(random01*MAX_SAFE) 打桩 4 例）；引擎层新增 `falaiServerBody`/`googleClientBody`/`zaiClientBody`/`openRouterBody`/`workersAiClientBody`/`replaceComfyWorkflow`（含 `bflBody` 共用的 `numStr` 数字语义 helper）。
- **✅ Comfy 全量对齐（generateComfyImageCommon L4231-L4261 + 服务端 /generate L562-L630）**：占位符搜索串带外层双引号 '"%prompt%"'、替换值 JSON.stringify 语义；%denoise%（undefined→1.0）/%clip_skip%（isNaN→-1 否则取负）/%vae%/%sampler%/%scheduler% 补齐；runPod 标志仅 steps/scale/width/height 4 项；删自创 model 兜底 v1-5-pruned-emaonly。App 直连 ComfyUI：提交 body = 官方模板字符串原样 '{"prompt": <workflow>}'（服务端 request.body.prompt 直传）；outputs images 平铺优先→gifs；/view query 不编码。登记偏差：comfy_placeholders 自定义替换无 UI（等价官方默认空数组）、%user_avatar%/%char_avatar% 头像注入未接。
- **✅ NovelAI/Horde/OpenAI 直连请求体逐项对齐**：NovelAI seed=sd_seed>=0 原样否则 Math.floor(rand*9999999999)（原恒随机且上界错）、负面链真实传递、scheduler 白名单归一 karras、use_order 移入 v4_prompt 内、model 对 '' 不兜底；Horde 参数全读设置 + karras 官方怪点恒 false（客户端从不发该字段）+ seed 仅 >=0 发送 + steps 无截断 + models 无 Deliberate 兜底；OpenAI model 读 sd_model + 模型族截断（1000/4000/32000）+ size 宽高比逻辑（1792/1536/512）+ quality/style/moderation 条件发送。登记偏差：NovelAI variety+/decrisper/sm/sm_dyn 开关无 UI（按官方默认 false/null 发送）、upscale 二次调用未接；OpenAI sora-2 视频分支未接；Horde sanitize 恒开（默认 true）与 nsfw=false 无开关 UI。
...
- 引擎测试 **全绿**（engine 全量 testcase + imagegen-services 63 + tts-services 37 + tts-local 38 + quickreply 16 + caption 17 + translate 19 + gallery-assets 5 + imagegen-prompt 59 → fixture 内 for-loop 累计）；App `:app:compileDebugKotlin` 全过；App 单测 `GalleryAssetsDiffTest` 5 例全绿；差分分组表 104 组，明细见 [docs/DIFF_MATRIX.md](DIFF_MATRIX.md)。
- 完成项对齐（“写了未差分”全部补齐）：
