...
| 斜杠异步命令 | 无异步执行器 | ✅ executeAsync + /gen /genraw（第 149 轮；/while 仍缺） |

**剩余已知半成品（继续治理中）**：工具调用 App 注册表、/while（循环命令，需循环语义；异步执行器第 149 轮已就绪）、表情精灵 App 层、ComfyUI 图像来源（需 workflow 管理）。
