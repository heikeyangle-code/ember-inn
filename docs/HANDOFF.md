...
| 人设 | `public/scripts/personas.js` | ✅ PersonaStore 官方全字段（name/description/title/avatarPath/lorebook/position/depth/role/connections）+ 聊天页顶栏常驻人设按钮（官方右侧抽屉等价）+ ⋮ 菜单人设：选择/新建/编辑/删除/复制/设为默认/锁定到本聊天（chat_metadata.persona）/头像选择/世界书选择；发送时 effectivePersona = 引擎 PersonaEngine.resolve（聊天锁 > 角色/群聊连接 > 默认 > 当前选择）；注入按官方 persona_description_positions：0=IN_PROMPT/2=TOP_AN/3=BOTTOM_AN（合并作者注释）/4=AT_DEPTH/9=NONE；lorebook = 人设关联世界书（官方 persona_lore_button/getPersonaLore：聊天/全局已激活跳过，否则并入扫描）已接 |
