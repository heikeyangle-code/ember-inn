...
宏 158 + pick 5 + 编辑器排序 6 + 自动执行选择 4 + 向量工具 14 + 角色卡 V2 归一 5 + 正则解析 9 + 作用域宏裁剪 7 + Anthropic 请求体 12 + Gemini 请求体 11 + 聊天历史填充 5 + 示例对话填充 4 + YAML 导入 5 + 提示词组装合并 7 + CharX 导入 9 + BYAF 纯逻辑 14 + BYAF 聊天导入 5 + BYAF 角色卡组装 4 + 名字规则 28 + 表情精灵 19 + 表情分类预处理 8 + 群聊激活 15 + 群聊角色卡 8 + 群聊深度提示 7 + 精灵存储 9 + 角色卡字段 8 + JSON 导入 10 + BYAF 完整导入 8 + 斜杠转义 13 + 提示词工具 9 + JSON 导出 6 = 525 例官方基准（全部通过）。
