...
| persona_sort_order | 无 UI | 人设设置补排序 |
| fuzzy_search | 部分 | 全域搜索接模糊权重 |
| confirm_message_delete | 无 UI | 行为设置补开关 |
| restore_user_input | 无 UI | 输入草稿全局存取（RA saveUserInput/restoreUserInput 同构） |
| custom_stopping_strings + _macro | 引擎有、无接线无 UI | BehaviorPrefs + 两调用点接线 + UI |
| show_tag_filters | 无 UI | 书架标签轨道开关（本 App 默认开） |
| aux_field | 无 UI | PosterTile 副标题（character_version/creator） |
| prefer_character_prompt / _jailbreak | 硬编码 true | ChatPromptFactory 覆盖门控 + UI 开关 |
| auto_load_chat | 已有 openLastChat | 已生效，hint 补官方键名 |
| 主题切换 lerp 400ms | 未做 | 动效阶段补 |
| CharacterDetail「角色主页」形态 | 未做 | 第 9 阶段重构 |
| bogus_folders 书架集合 | 未接 | 第 9 阶段接标签筛选 |
| sort_order 书架排序 | 无 UI | 第 9 阶段补 |
...
- 紧凑（<600dp）：现状 FloatHub 单栏。
- 中（600-840dp）：内容最大宽 600dp 居中，导航同紧凑。
- 展开（≥840dp）：导航轨 + **角色库 List-Detail**（列表|角色主页同屏）+ 聊天双栏（现状保留）。
- 实现：WindowSizeClass 口径（material3 adaptive 离线不可用时以 screenWidthDp 阈值自实现，
  行为等价，不破坏现有工程兼容性）。
