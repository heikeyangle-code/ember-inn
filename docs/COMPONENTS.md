...
| 消息 Markdown | **渲染内核 WebView**（assets/kernel/：showdown 2.1.0 + highlight 11.11.1 + DOMPurify 3.4.2 原版管线） | 自研内核（无第三方依赖），见 docs/HANDOFF.md §4 | 与官方 DOM/CSS 同构 → 官方与社区主题 CSS 原样生效 | 聊天消息行、流式正文；角色详情/世界书预览暂走 Compose 原生文本 | 官方 Showdown + highlight.js + DOMPurify（同版本同管线） |
